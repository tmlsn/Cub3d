...
/*   Updated: 2023/11/25 10:27:50 by tmalless         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_small_map(t_data *g)
{
	...
	while (g->m.map[i])
	{
		while (g->m.map[i][j])
		{
			if (g->m.map[i][j] == '1')
				fill_wall(g, i, j);
			else if (g->m.map[i][j] == '0')
				fill_floor(g, i, j);
			else if (g->m.map[i][j] == 'V')
				fill(g, i, j);
			j++;
		}
		j = 0;
...
int	get_i(t_data *g)
{
	...
	if (g->p.y / 32 < 4)
		i = 0;
	else if ((int)(g->p.y / 32 + 5) > g->m.height)
		i = g->m.height - 9;
	else
		i = g->p.y / 32 - 4;
	printf("i : %d\n", i);
...
int	get_j(t_data *g)
{
	...
	if (g->p.x / 32 < 4)
		j = 0;
	else if ((int)(g->p.x / 32 + 5) > g->m.width)
		j = g->m.width - 9;
	else
		j = g->p.x / 32 - 4;
	printf("j : %d\n", j);
...
{
	if (g->m.height < 9 && g->m.width < 9)
		print_small_map(g);
	else if (g->m.height < 9 && g->m.width < 9)
